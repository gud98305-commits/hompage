...
class ChatService:
    ...
    async def _handle_body_analysis(
        ...
        from body_analyzer import get_body_analyzer
    ...
    async def _handle_recommend(
        ...
        try:
            from product_adapter import get_products_by_curator
    ...
    async def _handle_game_items(
        ...
        from game_adapter import get_game_repo, GameDBError, GameItemToProductAdapter
        repo = get_game_repo()
        try:
    ...
    async def get_game_items(
        ...
        # Step 1. 게임 DB 조회
        from game_adapter import get_game_repo, GameDBError
        repo = get_game_repo()
        try:
            game_items = await repo.get_saved_items(
                user_id=user_id,
